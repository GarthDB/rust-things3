...
async fn main() -> Result<()> {
    ...
    // Run tests
    test_runner.run_all_tests(args.performance).await?;
...
impl TestRunner {
    ...
    async fn test_database_connectivity(&self) -> Result<()> {
        ...
        let tables = self.db.get_schema_info().await?;
    ...
    async fn test_basic_data_retrieval(&self) -> Result<()> {
        ...
        // Test inbox tasks
        let start = Instant::now();
        let inbox_tasks = self.db.get_inbox_tasks(Some(10)).await?;
        ...
        // Test projects
        let start = Instant::now();
        let projects = self.db.get_projects(None, Some(10)).await?;
        ...
        // Test areas
        let start = Instant::now();
        let areas = self.db.get_areas(Some(10)).await?;
        ...
        // Test today's tasks
        let start = Instant::now();
        let today_tasks = self.db.get_today_tasks(Some(10)).await?;
    ...
    async fn test_schema_validation(&self) -> Result<()> {
        ...
        let sample_task = self.db.get_inbox_tasks(Some(1)).await?;
    ...
    async fn run_performance_tests(&self) -> Result<()> {
        ...
        for (test_name, iterations) in test_cases {
            ...
            for i in 0..iterations {
                ...
                match test_name {
                    "inbox_query" => {
                        self.db.get_inbox_tasks(Some(50)).await?;
                    }
                    "projects_query" => {
                        self.db.get_projects(None, Some(50)).await?;
                    }
                    "areas_query" => {
                        self.db.get_areas(Some(50)).await?;
                    }
                    "today_query" => {
                        self.db.get_today_tasks(Some(50)).await?;
                    }
                    _ => unreachable!(),
                }
    ...
    fn validate_json_structure(&self, json: &Value, data_type: &str) -> Result<()> {
        if json.is_array() {
            let array = json.as_array().unwrap();
            info!("✓ {} JSON structure valid (array with {} items)", data_type, array.len());
