...
impl MetricsCollector {
    ...
    /// Collect database metrics
    #[instrument(skip(self))]
    async fn collect_database_metrics(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        if !is_connected {
            ...
            self.observability.record_error("database_connection", "Database connection lost");
        }
    ...
    #[instrument(skip(self))]
    async fn collect_application_metrics(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn collect_task_metrics(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let inbox_count = self.database.get_inbox(Some(1000)).map_err(|e| {
            error!("Failed to get inbox count: {}", e);
            e
        })?.len();
        
        let today_count = self.database.get_today(Some(1000)).map_err(|e| {
...
impl ErrorTracker {
    ...
    pub fn track_db_error(&self, operation: &str, error: &dyn std::error::Error) {
        ...
        let error_message = format!("Database operation '{}' failed: {}", operation, error);
    ...
    pub fn track_search_error(&self, query: &str, error: &dyn std::error::Error) {
        ...
        let error_message = format!("Search query '{}' failed: {}", query, error);
        self.track_error(error_type, &error_message);
    }
    ...
    pub fn track_export_error(&self, format: &str, error: &dyn std::error::Error) {
        ...
        let error_message = format!("Export in '{}' format failed: {}", format, error);
        self.track_error(error_type, &error_message);
    }
}
...
mod tests {
    ...
    fn test_performance_monitor_creation() {
        ...
        let monitor = PerformanceMonitor::new(observability);
    ...
    fn test_error_tracker_creation() {
        ...
        let tracker = ErrorTracker::new(observability);
