...
mod tests {
    ...
    #[tokio::test]
    async fn test_query_cache_search_query() {
        let cache = QueryCache::new_default();
        ...
        let query_key = "test_search_query";
        let params_hash = "test_params";

        // Test cache miss
        let result = cache
            .cache_search_query(query_key, params_hash, || async {
                Ok(search_results.clone())
            })
            .await
            .unwrap();

        assert_eq!(result.len(), search_results.len());
        ...
        assert_eq!(cached_result.len(), search_results.len());
    }

    #[tokio::test]
    async fn test_query_cache_ttl_expiration() {
        ...
        let initial_result = cache
            .cache_tasks_query(query_key, params_hash, || async {
                panic!("Should not execute fetcher on cache hit");
            })
            .await
            .unwrap();
        assert_eq!(initial_result.len(), tasks.len());

        // Wait for TTL to expire
        tokio::time::sleep(Duration::from_millis(200)).await;

        // Data should be expired (cache miss)
        let expired_result = cache
            .cache_tasks_query(query_key, params_hash, || async { Ok(create_mock_tasks()) })
            .await
            .unwrap();
        assert_eq!(expired_result.len(), tasks.len());
    }

    #[tokio::test]
    async fn test_query_cache_max_entries() {
        let config = QueryCacheConfig {
        ...
        for i in 0..3 {
            ...
            let params_hash = format!("params_{}", i);
        ...
        let first_result = cache
            .cache_tasks_query("query_0", "params_0", || async { Ok(create_mock_tasks()) })
            .await
            .unwrap();
        assert_eq!(first_result.len(), tasks.len());
    }

    #[tokio::test]
    async fn test_query_cache_max_result_size() {
        let config = QueryCacheConfig {
            ttl: Duration::from_secs(60),
            max_entries: 100,
            max_result_size: 100, // Very small size limit
            enable_compression: false,
        };
        let cache = QueryCache::new(config);

        let large_tasks = vec![Task {
            uuid: Uuid::new_v4(),
            ...
            notes: Some("x".repeat(1000)), // Large notes
            tags: vec!["tag1".to_string(), "tag2".to_string()],
            start_date: Some(Utc::now().date_naive()),
            deadline: Some(Utc::now().date_naive()),
            start_time: Some(Utc::now().time()),
            stop_time: Some(Utc::now().time()),
        }];

        let query_key = "large_query";
        let params_hash = "test_params";

        // Should not cache due to size limit
        let result = cache
            .cache_tasks_query(query_key, params_hash, || async { Ok(large_tasks.clone()) })
            .await
            .unwrap();

        assert_eq!(result.len(), large_tasks.len());

        // Should be a cache miss (not cached due to size)
        let miss_result = cache
            .cache_tasks_query(query_key, params_hash, || async { Ok(create_mock_tasks()) })
            .await
            .unwrap();

        assert_eq!(miss_result.len(), 2); // create_mock_tasks returns 2 tasks
    }
    ...
    async fn test_query_cache_compression() {
        ...
        let tasks = create_mock_tasks();
        let query_key = "compressed_query";
        let params_hash = "test_params";
    ...
    #[tokio::test]
    async fn test_query_cache_invalidate_by_entity() {
        let cache = QueryCache::new_default();
        ...
        cache.invalidate_by_entity("task", Some(&tasks[0].uuid));
    ...
    #[tokio::test]
    async fn test_query_cache_create_project_dependencies() {
        let cache = QueryCache::new_default();
        ...
        let dependencies = cache.create_project_dependencies(&projects);

        assert!(!dependencies.is_empty());
        assert!(dependencies.iter().any(|dep| dep.table == "TMProject"));
    }

    #[tokio::test]
    async fn test_query_cache_create_area_dependencies() {
        let cache = QueryCache::new_default();
        ...
        let dependencies = cache.create_area_dependencies(&areas);

        assert!(!dependencies.is_empty());
        assert!(dependencies.iter().any(|dep| dep.table == "TMArea"));
    }

    #[tokio::test]
    async fn test_query_cache_calculate_result_size() {
        let cache = QueryCache::new_default();

        let tasks = create_mock_tasks();
        let size = cache.calculate_result_size(&tasks);
    ...
    async fn test_query_cache_concurrent_access() {
        ...
        for i in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                let query_key = format!("concurrent_query_{}", i);
                let params_hash = format!("params_{}", i);
                let tasks = create_mock_tasks();
    ...
    async fn test_query_cache_stats() {
        ...
        let miss_stats = cache.get_stats();
